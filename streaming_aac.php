...
<? 
$list_package=$tb_core_packages->select("package_id,name,price_monthly,price_quarterly,module,price_semiannual,price_yearly,price_2yearly,price_3yearly","company_id='$_SESSION[core_company_id]' AND type='shared' AND name IN ('SS1 AAC','SS2 AAC','SS3 AAC','SS4 AAC') Order by price_monthly ASC");
$row_package=mysql_fetch_assoc($list_package);
?>
<div style="overflow:hidden; ">
    
        <div style="color:#333; font-size:26px; font-weight:900px;font-family:'Trebuchet MS', Arial, Helvetica, sans-serif;text-shadow: 0 1px 0 #EAEAEA; margin-top:10px;">Streaming en AAC+ Plus V2 - tu Radio en internet</div>
        <div style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:13px; color:#333;text-shadow: 0 1px 0 #F9F9F9; margin-top:15px; margin-bottom:25px; line-height:20px;"> 
          <p>Streaming de Audio en Alta definición (AAC+ Plus V2), estos planes son ideales para transmitir a grandes cantidades de usuarios de manera simultanea a una excelente calidad de audio (Superior a 128kbps calidad CD) pero a un bitrate menor, por lo tanto usted no requiere de una conexión de subida veloz y a su vez sus oyentes tampoco requieren una conexión de bajada veloz, todo esto gracias a nuestro avanzado y exclusivo reproductor en flash que ofrecemos gratis en cualquier plan.            </p>
          <p>Por ejemplo: Usted transmitirá a 32Kbps y pero la calidad del audio será igual o superior que si transmitiera a 96Kbps, transmitiendo a 48Kbps la calidad del audio será igual o superior a 128Kbps (Calidad de un CD).</p>
	 <p>Nota Importante: La configuración del shoutcast para la transmisión desde su PC deberá ser de acuerdo al bitrate contratado en el paquete, esto evitará que utilice mayor recursos del 
servidor y su cuenta sea bloqueada por el sistema durante 24 horas.</p>
    </div>
                    	...
                    	<div style="height:465px; margin-left:5px; margin-right:5px; border:solid 1px #ECECEC;" class="redondeo10px">
                            <div style="height:50px; background-color:#333; margin-bottom:15px;" class="redondeo5px">
                                <div style="float:left; padding-left:5px; padding-top:9px; overflow:hidden; width:185px;">
                                    <div style="float:left;"><img src="images/radio_32.png" alt=""  /></div>
                             ...
                             <div style="border-bottom:solid 1px #F6F6F6; margin-left:5px; margin-right:5px;margin-bottom:7px;">
                               ...
                               <div style="color:#06C; font-size:16px; font-weight:800;">hasta <?=$tb_page_packages->obtenerDato("domain_hosted","package_id_core=".$row_package['package_id']);?> oyentes</div>
                            </div>
                            <div style="border-bottom:solid 1px #F6F6F6; margin-left:5px; margin-right:5px;margin-bottom:7px;">
                            	<div style="color:#000; font-weight:700;">Calidad de Bitrate:</div>
                                <div style="color:#930; font-size:14px;"><?=$tb_page_packages->obtenerDato("bandwidth","package_id_core=".$row_package['package_id']);?> kbps</div>
                            </div>
                            <div style="border-bottom:solid 1px #F6F6F6; margin-left:5px; margin-right:5px;margin-bottom:7px;">
                            	<div style="color:#000; font-weight:700;">Incluye AutoDj:</div>
                            ...
                            <div style="border-bottom:solid 1px #F6F6F6; margin-left:5px; margin-right:5px; margin-bottom:7px; background-color:#FFF;">
                            	<div style="color:#000; font-weight:700;">Espacio para AutoDj</div>
                            ...
                            <div><input type="hidden" name="service_type" value="streaming_shared" /></div>
	...
	<!-- Inicia DIV -->
     <? 
			...
			include("modules_service/module_castcontrol_radio.php");
          	include("modules_service/module_support24-7.php");
            include("modules_service/module_core.php");
            include("modules_service/module_supplements_aac.php");
            include("modules_service/module_user_manual_aac.php");
