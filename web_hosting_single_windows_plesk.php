...
<? 
$list_package=$tb_core_packages->select("package_id,name,price_monthly,price_quarterly,module,price_semiannual,price_yearly,price_2yearly,price_3yearly","company_id='$_SESSION[core_company_id]' AND type='shared' AND module='plesk' Order by price_monthly ASC");
$row_package=mysql_fetch_assoc($list_package);
?>
<div style="overflow:hidden; ">
    
        <div style="color:#333; font-size:26px; font-weight:900px;font-family:'Trebuchet MS', Arial, Helvetica, sans-serif;text-shadow: 0 1px 0 #EAEAEA; margin-top:10px;">Web Hosting con Plesk</div>
        <div style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:13px; color:#333;text-shadow: 0 1px 0 #F9F9F9; margin-top:15px; margin-bottom:25px; line-height:20px;"> Plesk es un panel de Control que hemos instalado sobre servidores con sistema operativo Windows, por lo tanto tus aplicaciones desarrolladas para ésta tecnología, aquí se ejecutarán sin ningún problema.</div>
    
    
	<div id="slider_contenedor_fijo" style="width:930px; height:510px; float:left; overflow:hidden; margin-left:5px;">
    	<div style="height:510px; position:relative; width:<?=310*$tb_page_packages->math("COUNT","core_company_id='$_SESSION[core_company_id]' AND link_id='$_SESSION[link_id]'");?>px; left:0px;" id="home_slide_pack">
	<?
    do{
        if($tb_page_packages->math("COUNT","package_id_core=".$row_package['package_id'])>0){
                ?>
                	<div style="width:310px;float:left;height:510px;">
                    	<div style="height:505px; margin-left:5px; margin-right:5px; border:solid 1px #ECECEC;" class="redondeo10px">
                            ...
                            <div style="border-bottom:solid 1px #F6F6F6; margin-left:5px; margin-right:5px;margin-bottom:7px;">
                                ...
                                <div style="color:#06C; font-size:14px;">soporta hasta <?=$tb_page_packages->obtenerDato("domain_hosted","package_id_core=".$row_package['package_id']);?> dominios adicionales</div>
                            </div>
                            
                            <div style="border-bottom:solid 1px #F6F6F6; margin-left:5px; margin-right:5px;margin-bottom:7px;">
                            	<div style="color:#000; font-weight:700;">Cuentas de Correo:</div>
                                <div style="color:#06C; font-size:11px;">hasta <strong style="color:#F63;"><?=$tb_page_packages->obtenerDato("email_accounts","package_id_core=".$row_package['package_id']);?></strong> cuentas de correo.<br />
                        con espacio máximo de <strong style="color:#F63;"><?=$tb_page_packages->obtenerDato("email_accounts_maxspace","package_id_core=".$row_package['package_id']);?>MB</strong> c/u.</div>
                            </div>
                              <div style="border-bottom:solid 1px #F6F6F6; margin-left:5px; margin-right:5px;margin-bottom:7px;">
                            	<div style="color:#000; font-weight:700;">MySQL Bases de Datos</div>
                            	<div style="color:#06C; font-size:11px;">
									están permitidas <strong style="color:#F63;"><?=$tb_page_packages->obtenerDato("mysql_space","package_id_core=".$row_package['package_id']);?> </strong> bases de datos. <br />
                       				en un espacio de <strong style="color:#F63;"><?=$tb_page_packages->obtenerDato("mysql_total_dbs","package_id_core=".$row_package['package_id']);?>MB</strong>
								</div>
                            </div>
                            <div style="border-bottom:solid 1px #F6F6F6; margin-left:5px; margin-right:5px;margin-bottom:7px;">
                            	<div style="color:#000; font-weight:700;">MS SQL Bases de Datos</div>
                            	<div style="color:#06C; font-size:11px;">
									están permitidas <strong style="color:#F63;"><?=$tb_page_packages->obtenerDato("mssql_space","package_id_core=".$row_package['package_id']);?> </strong> bases de datos. <br />
                       				en un espacio de <strong style="color:#F63;"><?=$tb_page_packages->obtenerDato("mssql_total_dbs","package_id_core=".$row_package['package_id']);?>MB</strong>
								</div>
                            </div>
                            ...
                            <div><input type="hidden" name="service_type" value="windows_shared" /></div>
	...
	<!-- Inicia DIV -->
     <? 
			...
			include("modules_service/module_plesk_user.php");
		include("modules_service/module_application_value.php");
