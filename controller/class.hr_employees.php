...
class hr_employees {
	function select($parametro="*",$condicion=NULL){
		...
		$selectSQL="SELECT $parametro FROM `hr_employees` $condicion";
	...
	function obtenerFila($parametro,$condicion=NULL){
		include("Connections/db_connection_page.php");
			if($condicion==NULL)
				...
				$condicion=" WHERE ".$condicion;
		$selectSQL="SELECT $parametro FROM `hr_employees` $condicion Limit 1";
		$result=mysql_query($selectSQL,$db_connection);
	...
	function obtenerDato($parametro,$condicion=NULL){
		...
		$condicion=" WHERE ".$condicion;
		$selectSQL="SELECT $parametro AS Dato FROM `hr_employees` $condicion  Limit 1";
		$result=mysql_query($selectSQL,$db_connection);
		$row= mysql_fetch_assoc($result);
		return $row['Dato'];
	}
	function math($operador,$condicion=NULL){
		...
		$condicion=" WHERE ".$condicion;
		$selectSQL="SELECT $operador(*) AS Result FROM `hr_employees` $condicion";
		$result=mysql_query($selectSQL,$db_connection);
		$row= mysql_fetch_assoc($result);
		return $row['Result'];
		}
